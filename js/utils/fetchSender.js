    ...
    async sendData(data, options = {}) {
      ...
      for (let attempt = 1; attempt <= config.retries; attempt++) {
        try {
          ...
          console.log(`❌ FetchSender: Attempt ${attempt} failed:`, error.message);
          ...
          if (attempt < config.retries) {
            console.log(`⏳ FetchSender: Retrying in ${config.retryDelay}ms...`);
      ...
      // All attempts failed
      console.error('💥 FetchSender: All attempts failed. Last error:', lastError);
