...
const chatContainer = await window.DOMAPI.getElement(
  '[role="main"]',
  "outerHTML"
);
...
await window.DOMAPI.setAttribute("#my-input", {
...
```javascript
// Create new element
const newElement = window.DOMHelpers.createElement(
  "div",
  {
    id: "my-new-element",
...
// Insert element
const inserted = window.DOMHelpers.insertElement(
  "#container",
  newElement,
  "append"
);
...
async function extractPageContent() {
  try {
    ...
    // Get all headings
    const headings = window.DOMHelpers.getAllElements(
      "h1, h2, h3, h4, h5, h6"
    ).map((h) => ({ level: h.tagName, text: h.textContent }));

    // Get all links
    const links = window.DOMHelpers.getAllElements("a[href]").map((a) => ({
      text: a.textContent,
      url: a.href,
    }));
...
async function enhanceChatGPTUI() {
  try {
    ...
    const toolbar = window.DOMHelpers.createElement("div", {
      id: "custom-toolbar",
      style:
        "position: fixed; top: 10px; right: 10px; background: #f0f0f0; padding: 10px; border-radius: 5px; z-index: 9999;",
    });

    // Add buttons to toolbar
    const saveButton = window.DOMHelpers.createElement(
      "button",
      {
    ...
    const clearButton = window.DOMHelpers.createElement(
      "button",
      {
    ...
    saveButton.addEventListener("click", async () => {
      const chatContent = await window.DOMAPI.getElement(
        '[role="main"]',
        "text"
      );
